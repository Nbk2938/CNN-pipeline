...
class DenoisingLoss(nn.Module):
    """0.6*MSE + 0.2*L1 + 0.2*FFT-magnitude MSE.

    The FFT term penalises residual periodic structure (grid artifacts)
    that spatial losses are blind to.
    """

    def __init__(self, mse_w: float = 0.6, l1_w: float = 0.2, fft_w: float = 0.2) -> None:
        super().__init__()
...
class DenoisingDataset(Dataset):
    ...
    def _augment(
        ...
        # Circular shift: randomises the absolute pixel position of the grid artifact.
        # torch.roll is seamless — no boundary discontinuity — and the FFT of a
        # circularly-shifted image has the same magnitude spectrum, so the FFT loss
        # is unaffected by the shift itself.
        ...
        # Noise level augmentation: randomly scale noise intensity by ±30%.
        # Exposes the model to heavier corruption, improving generalisation to
        # extreme cases like test sample 42.
        if clean is not None and rng.random() < 0.5:
...
def denoise_tta(
    ...
    """Test-time augmentation: average predictions over 8 flip/rotation variants."""
    model.eval()
    preds = []
    for k in range(4):
        for flip in (False, True):
            img = np.rot90(noisy_2d, k)
            if flip:
                img = np.fliplr(img)
